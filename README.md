...
там (кроме картинок). \
В проекте есть готовый файл по разворачиванию базы данных (**deploy_database.py**), файл с данными для 
викторины (**questions_db.json**) и фото животных, которые будут отображаться с текстом 
после прохождения викторины (**modul_quiz/foto**), чтобы бота можно было легко запустить. \
Если созавать базу отдельно, и если не добавить в базу вопросов - модуль с викториной работать не
будет.
